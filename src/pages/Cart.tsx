...
useEffect(() => {
  const fetchCartItems = async () => {
    try {
      ...
      // Check if response.data contains cartItems
      if (response.data.cartItems.length === 0) {
        setShowEmptyCartMessage(true);
        setCartItems([]);
        setTotalPrice(0); 
      } else {
        // Set cart items and total price if cart is not empty
  ...
  return (
    <Container className='mt-5'>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="mt-3 mb-0">Cart</h1>
         { !showEmptyCartMessage && <Button variant="primary" onClick={handleCheckout}>
            Checkout
          </Button>}
      </div>
